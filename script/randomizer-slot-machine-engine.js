class RandomizerSlotMachineEngine {
    ...
    _randomizeAndGetItemAddons(role, character, language) {
        this._randomizeItemAddons(role, character);
        if(language === 'fr') {
            return this.characterAddonSlotMachineResult.frenchNames;
        }
    ...
    _randomizeAndGetOffering(role, language) {
        this._randomizeOffering(role);
        
        if(language === 'fr') {
            return this.offeringSlotMachineResult.frenchName;
        }
    ...
    randomizeItems() {
        ...
        let language = this.uiGenerator.uiTranslator.locale;

        switch(role) {
            case ROLES[0]:
                this._randomizeActiveItemAddons(role, character, language);
                break;
            case ROLES[1]:
                this._randomizeActiveSurvivorItem(language);
                break;
        }
    }
    ...
    randomizeOfferings() {
        ...
        let offering = this._randomizeAndGetOffering(role, language)
    ...
    _randomizeActiveSurvivorItem(language) {
        ...
        let self = this;
        setTimeout(function () {
            self._randomizeActiveItemAddons(ROLES[1], item.type, language);
    ...
    _randomizeActiveItemAddons(role, item, language) {
        ...
        let addons = this._randomizeAndGetItemAddons(role, item, language);
